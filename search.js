/*Javascript code for SHEP AI website: shepai.github.io
*Code written by Dexter RC Shepherd, aged 18
*This code will provide all the needed areas for the website from

	-a search bar
	-a demo word splitter
	-a FAQ chatbot
	
*Code can be re-used with the permission of the SHEP AI project leader and primary developer, Dexter Shepherd
*for use in websites. A package will be developed so users can use this software on their websites
*The answers will be generated by a Python program
*/
...
/////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function enterMessage()
	{
		...
		var add='<br id="'+num+'"><div id="'+num+'" class="userMessage">'+text+'</div><div id="'+num+'" class="msgBrk"></div>';
				
		num=num+1;
		output=enterData(text);
		var add2='<br id="'+num+'"><div id="'+num+'" class="shepMessage">'+output+'</div><div id="'+num+'" class="msgBrk"></div>';
		num=num+1;
		
		document.getElementById("mainContent").innerHTML+=add+add2;
	...
	function enterData(uservalue)
	{
		
		//gather types of data
		var sentence=(uservalue).split(" ");//split to sentences
		var sig=[];//significant words
		var threshold=getPop();
		console.log("threshold of significance: "+threshold);
		for(i=0;i<sentence.length;i++){ //loop through every word
		...
		console.log(sig);console.log(other); //output for debuggin purposes
		//check files for subjects
		//check root
		//if data output it
		//if no data then give automatic response and contact info
		
		//set a did that answer question analyzer
	}
	var loadedFile="file{shep,download}shep{what=SHEP is an open source project,who=SHEP is an open source project founded by Dexter Shepherd}download{how=You can find our downloads at shepai.github.io/downloads}";
	...
	function breakItem(data,type) //split data and find the type
	{
		//loop through information and find the info.
		var type1=type+"{";
		var found=-1;
		for(i=0;i<data.length;i++) { //loop through to find where data is found
			var count=0;
			for(j=0;j<type1.length;j++)
			{
				if(i+j<data.length)
				{
					if(type1.charAt(j)==data.charAt(i+j))
					{
						count++;
					}
				}
			}
			if(count==type1.length){found=i;}
		}
		var start=found+type1.length;
		var cha="";var innerdata="";
		while(cha!="}"){innerdata+=cha;cha=data.charAt(start);start++;}
		return innerdata;
	}
	function searchTop(data,items)
	{
		var arr = data.split(","); //split the gathered data
		var found=[];
		for(i=0;i<items.length;i++) //loop through items entered
		{
			for(j=0;j<arr.length;j++) //loop through current data
			{
				var dat=arr[j].split("=");
				for(k=0;k<dat.length/2;k++)
				{
					if(items[i]==dat[2*k])
					{
						found.push(dat[(2*k)+1]); //add relevant info to the data
					}
				}
			}
		}
